...
int addNode(char * name, pid_t *propid) {
  struct myNode *temp = (struct node *)malloc(sizeof(struct node));
  if(!temp) {
...
*/
struct node *findNode(pid_t *targetpid){
  if(!head)
    return NULL;
...
* if found, a pointer to the node is returned
*/
struct node *findNode(char *targetname){
  if(!head)
    return NULL;
  struct node *temp = head;
  /* reset the last node's next pointer to be NULL
...
void deleteNode(char *deletename) {
  struct node *target = findNode(deletepid);
  if(!target) {
    return;
  }
...
*/
void deleteNode(char *deletename) {
  struct node *target = findNode(deletename);
  if(!target) {
    return;
  }
  if(target->next == target) { /*if it is a single node */
